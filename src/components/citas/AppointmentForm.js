...
const AppointmentForm = ({
  ...
  const [serviceDuration, setServiceDuration] = useState(
    selectedService.timeBlocks || services[0].timeBlocks
  );
  const [textDuration, setTextDuration] = useState(
    selectedService.duration || services[0].duration
  );
      ...
      <AppointmenFields onSubmit={handleSubmit}>
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => {
              setServiceDuration(
                e.target.selectedOptions[0].dataset.timeblocks
              );
        ...
        <div>
          ...
          />
          <span className="date">
            {moment(dateFieldToEpoch(values.date) + 20000000).format("dddd LL")}
          </span>
        </div>
        <br />
        <small>
          * Una vez completados los datos selecciona la hora que deseas
        </small>
        {user?.admin && (
          <AdminDashboard
